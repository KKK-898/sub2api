...
type Group struct {
	...
	// Whether to strip Codex/Responses image generation tools for this OpenAI group
	StripCodexImageGenerationTool bool `json:"strip_codex_image_generation_tool,omitempty"`
	// 是否允许该分组使用批量图片生成能力
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldStripCodexImageGenerationTool:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field strip_codex_image_generation_tool", values[i])
			} else if value.Valid {
				_m.StripCodexImageGenerationTool = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("strip_codex_image_generation_tool=")
	builder.WriteString(fmt.Sprintf("%v", _m.StripCodexImageGenerationTool))
	builder.WriteString(", ")
