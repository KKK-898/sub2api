...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if stripImageGenerationTool {
		...
		if changed {
			...
			requestView = newOpenAIRequestView(body)
			reqModel, reqStream, promptCacheKey = requestView.Model, requestView.Stream, requestView.PromptCacheKey
			logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Stripped /responses image-generation tools by effective account/group policy")
		}
	}
	...
	codexImageGenerationBridgeEnabled := isCodexCLI && imageGenerationAllowed && !stripImageGenerationTool && s.isCodexImageGenerationBridgeEnabled(ctx, account, apiKey)
