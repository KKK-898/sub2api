...
func TestOpenAIGatewayServiceForward_GroupPolicyStripsAdvertisedImageToolsBeforeGate(t *testing.T) {
	gin.SetMode(gin.TestMode)

	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_group_strip","model":"gpt-5.4","usage":{"input_tokens":2,"output_tokens":1}}`)),
	}}
	svc := newOpenAIImageGenerationControlTestService(upstream)
...
func TestOpenAIGatewayServiceForward_GroupStripOverridesBridgeInjection(t *testing.T) {
	gin.SetMode(gin.TestMode)

	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_no_inject","model":"gpt-5.4","usage":{"input_tokens":1,"output_tokens":1}}`)),
	}}
	svc := newOpenAIImageGenerationControlTestService(upstream)
...
func TestOpenAIGatewayServiceForward_GroupPolicyStripsToolsForPassthrough(t *testing.T) {
	gin.SetMode(gin.TestMode)

	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_passthrough_strip","model":"gpt-5.4","usage":{"input_tokens":2,"output_tokens":1}}`)),
	}}
	svc := newOpenAIImageGenerationControlTestService(upstream)
	c, _ := newOpenAIImageGenerationControlTestContext(false, "third-party-client/1.0")
	apiKey := getAPIKeyFromContext(c)
	apiKey.Group.StripCodexImageGenerationTool = true
	account := newOpenAIImageGenerationControlTestAccount()
