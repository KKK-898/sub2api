...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(raw []byte) (openAIWSClientPayload, error) {
		...
		explicitImageIntent := IsExplicitImageGenerationIntent(openAIResponsesEndpoint, originalModel, normalized)
		if stripImageGenerationTool {
			if stripped, changed, stripErr := stripOpenAIImageGenerationToolFromRawPayload(normalized); stripErr != nil {
				...
				normalized = stripped
				logOpenAIWSModeInfo("ingress_ws_codex_image_tool_stripped_by_effective_policy account_id=%d", account.ID)
			}
		}
		codexBridgeEnabled := isCodexCLI && imageGenerationAllowed && !stripImageGenerationTool && s.isCodexImageGenerationBridgeEnabled(ctx, account, apiKey)
