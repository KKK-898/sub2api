...
func TestAdminService_CreateGroup_NormalizesCodexImageToolStripByPlatform(t *testing.T) {
	t.Run("openai preserves strip flag", func(t *testing.T) {
		repo := &groupRepoStubForAdmin{}
		svc := &adminServiceImpl{groupRepo: repo}
		...
		require.NoError(t, err)
		require.True(t, group.StripCodexImageGenerationTool)
		require.True(t, repo.created.StripCodexImageGenerationTool)
	})

	t.Run("non-openai clears strip flag", func(t *testing.T) {
		repo := &groupRepoStubForAdmin{}
		svc := &adminServiceImpl{groupRepo: repo}
		...
		require.NoError(t, err)
		require.False(t, group.StripCodexImageGenerationTool)
		require.False(t, repo.created.StripCodexImageGenerationTool)
	})
}
...
func TestAdminService_UpdateGroup_ClearsCodexImageToolStripOutsideOpenAI(t *testing.T) {
	existingGroup := &Group{
		ID:                            1,
		Name:                          "openai-text-only",
		Platform:                      PlatformOpenAI,
	...
	group, err := svc.UpdateGroup(context.Background(), 1, &UpdateGroupInput{Platform: PlatformAnthropic})

	require.NoError(t, err)
	require.False(t, group.StripCodexImageGenerationTool)
	require.False(t, repo.updated.StripCodexImageGenerationTool)
}
