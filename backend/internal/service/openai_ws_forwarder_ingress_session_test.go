...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_GroupStripRemovesExplicitImageChoiceBeforeGate(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	captureConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_group_image_strip","model":"gpt-5.4","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	captureDialer := &openAIWSCaptureDialer{conn: captureConn}
	pool := newOpenAIWSConnPool(cfg)
	...
	account := &Account{
		ID:          32,
		Name:        "openai-group-image-strip-ws",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeOAuth,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeCtx, cancelWrite := context.WithTimeout(context.Background(), 3*time.Second)
	err = clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.4","stream":false,"input":"write code","tools":[{"type":"function","name":"shell","parameters":{"type":"object"}},{"type":"namespace","name":"image_gen"}],"tool_choice":{"type":"namespace","name":"image_gen"}}`))
	cancelWrite()
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, coderws.MessageText, msgType)
	require.Equal(t, "resp_group_image_strip", gjson.GetBytes(message, "response.id").String())
	...
	case serverErr := <-serverErrCh:
		require.NoError(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("waiting for image-strip websocket ingress to finish timed out")
	}

	require.Len(t, captureConn.writes, 1)
	upstreamPayload := requestToJSONString(captureConn.writes[0])
	require.False(t, gjson.Get(upstreamPayload, `tools.#(type=="image_generation")`).Exists())
	require.False(t, gjson.Get(upstreamPayload, `tools.#(name=="image_gen")`).Exists())
